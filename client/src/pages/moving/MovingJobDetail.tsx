...
import { ArrowLeft, Plus, Trash2, FileText, MapPin, Users, Truck as TruckIcon, AlertCircle, Package, Star, Wrench, Camera, X, Tag, CheckCircle2, Pencil, Image as ImageIcon, Check, Share2, Copy, ExternalLink, CalendarCheck } from 'lucide-react'
    ...
    <div style={{ background: '#FDFCFA', borderRadius: 20, border: '1px solid rgba(20,8,31,0.06)' }} className="p-5 sm:p-7 space-y-8 pb-24 md:pb-7">
      ...
      <Modal open={shareModal} title="Share Upload Link" onClose={() => setShareModal(false)}>
        <div className="space-y-4">
          ...
          <a
            href={`https://wa.me/${(job.customer?.phone || '').replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`Hi ${job.customer?.fullName?.split(' ')[0] || ''},\n\nPlease upload photos and videos of your items for your upcoming move using this link:\n\n${shareLink}\n\nThis helps us prepare better for your move.\n\nThank you,\nPurpleBox Move`)}`}
            target="_blank"
      ...
      <Modal open={visitModal} title="Schedule Visit" onClose={() => { setVisitModal(false); setVisitLinkCopied(false) }}>
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Share this portal link with the customer. They can log in with their phone number and upload photos, videos, and notes for this job.
          </p>
          <div className="flex items-center gap-2">
            <Input value={`${window.location.origin}/portal`} readOnly className="text-xs font-mono" />
            <Button
              size="sm"
              variant="outline"
              onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/portal`); setVisitLinkCopied(true); setTimeout(() => setVisitLinkCopied(false), 2000) }}
              className="shrink-0"
            >
              {visitLinkCopied ? <><Check size={14} className="mr-1" /> Copied</> : <><Copy size={14} className="mr-1" /> Copy</>}
            </Button>
          </div>
          <a
            href={`https://wa.me/${(job.customer?.phone || '').replace(/[^0-9]/g, '')}?text=${encodeURIComponent(`Hi ${job.customer?.fullName?.split(' ')[0] || ''},\n\nPlease upload photos and videos of your property for your upcoming move. Use this link to access your portal:\n\n${window.location.origin}/portal\n\nLog in with your phone number and go to job ${job.jobNo} to upload.\n\nThank you,\nPurpleBox Move`)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 w-full py-2.5 rounded-lg text-white text-sm font-medium"
            style={{ background: '#25D366' }}
          ...
          <div className="flex justify-end pt-4 border-t">
            <Button variant="outline" onClick={() => { setVisitModal(false); setVisitLinkCopied(false) }}>Close</Button>
          </div>
        </div>
      </Modal>
