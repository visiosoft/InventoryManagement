...
export default function SignMovingJob() {
  const { token } = useParams<{ token: string }>()

  const [info, setInfo]               = useState<MovingJobInfo | null>(null)
  ...
  useEffect(() => {
    fetch(`${apiBase}/sign-moving/${token}`)
      .then((r) => r.json())
      .then((data) => {
      ...
      .catch(() => setLoadErr('Could not load job details'))
  }, [token])
  ...
  async function handleSign() {
    ...
    setSubmitErr('')
    try {
      const res = await fetch(`${apiBase}/sign-moving/${token}`, {
        method: 'POST',
  ...
  if (!info) {
    return (
      <PageShell>
        <div style={{ textAlign: 'center', padding: 60, color: '#888' }}>Loading move details…</div>
      </PageShell>
    )
  }
    ...
    return (
      <PageShell>
        <div style={{ textAlign: 'center', padding: '60px 20px' }}>
          ...
          <p style={{ color: '#666' }}>Move <strong>{info.jobNo}</strong> has already been signed.</p>
        </div>
      </PageShell>
    )
  }
    ...
    return (
      <PageShell>
        <div style={{ textAlign: 'center', padding: '60px 20px' }}>
          ...
          <h2 style={{ fontSize: 24, fontWeight: 700, marginBottom: 12 }}>Agreement signed!</h2>
          <p style={{ color: '#555', marginBottom: 24, maxWidth: 400, margin: '0 auto 24px' }}>
            Thank you, <strong>{signerName}</strong>. Your moving agreement for <strong>{info.jobNo}</strong> is confirmed.
  ...
  const pdfUrl = `${apiBase}/sign-moving/${token}/pdf#view=FitH`

  return (
    <PageShell>
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '24px 16px' }}>
        <div style={{ marginBottom: 24 }}>
          <h1 style={{ fontSize: 22, fontWeight: 700, marginBottom: 4 }}>Moving Services Agreement</h1>
          <p style={{ color: '#666', fontSize: 14 }}>
            {info.jobNo} · Please read the document below and sign to confirm your agreement.
          </p>
        </div>
          ...
          {/iPhone|iPad|iPod/i.test(navigator.userAgent) ? (
            <div style={{ padding: '32px 16px', textAlign: 'center', background: '#fafafa' }}>
              <p style={{ color: '#555', fontSize: 14, marginBottom: 16 }}>
                PDF preview is not available on iOS. Please open it in a new tab to read the agreement before signing.
              </p>
              <a
                href={pdfUrl}
                target="_blank"
        ...
        <div style={{ background: '#fff', border: '1px solid #e5e5e5', borderRadius: 12, padding: 24 }}>
          ...
          {mode === 'draw' ? (
            <div style={{ marginBottom: 16 }}>
              ...
              <DrawCanvas onCapture={setSigUrl} />
          ...
          >
            {busy ? 'Signing…' : '✅ Sign moving agreement'}
          </button>

          <p style={{ fontSize: 12, color: '#aaa', textAlign: 'center', marginTop: 12 }}>
            This link expires {fmt(info.expiresAt)}. Powered by PurpleBox Moving.
          </p>
        </div>
      </div>
    </PageShell>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f4f4f7', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <header style={{ background: '#1a1a2e', padding: '14px 24px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <span style={{ color: '#fff', fontWeight: 600, fontSize: 16 }}>PurpleBox Moving</span>
      </header>
      <main>{children}</main>
    </div>
  )
