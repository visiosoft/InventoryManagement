                    ...
                    <div className="space-y-5">
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <button type="button" onClick={addExtra} className="text-xs text-primary hover:underline cursor-pointer font-medium">+ Add row</button>
                            </div>
                            <div className="rounded-lg border overflow-hidden">
                                ...
                                <div className="divide-y">
                                    {items.map((it, idx) => (
                                        <div key={idx} className="grid grid-cols-[1fr_100px_70px_32px] gap-2 px-3 py-2 items-center">
                                            <Input value={it.itemDetails} onChange={e => updateDesc(idx, e.target.value)} placeholder="Service" className="text-sm" />
                                            <Input type="number" min={0} step="1" value={it.amount} onChange={e => updateAmount(idx, Number(e.target.value))} className="text-sm text-right" />
                                            <Input type="number" min={0} max={100} step="1" value={it.discountPct || 0} onChange={e => updateDiscount(idx, Number(e.target.value))} className="text-sm text-center" />
                                            <button type="button" onClick={() => removeItem(idx)} className="text-muted-foreground hover:text-destructive cursor-pointer"><X size={14} /></button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="rounded-xl border bg-muted/30 p-4 space-y-2">
                            ...
                            <div className="space-y-1 text-sm">
                                {items.filter(it => it.amount > 0).map((it, i) => (
                                    <div key={i} className="flex justify-between gap-2">
                                        <span className="truncate text-muted-foreground">{it.itemDetails || `Item ${i + 1}`}</span>
                                        <span className="shrink-0">{formatMoney(it.amount)}{(it.discountPct || 0) > 0 ? <span className="text-destructive text-xs ml-1">-{it.discountPct}%</span> : ''}</span>
                                    </div>
                                ))}
                            </div>
