...
router.post('/request-move', async (req, res) => {
  try {
    const { serviceType, propertyType, pickupAddress, pickupFloor, pickupElevator, deliveryAddress, deliveryFloor, deliveryElevator, moveDate, timeSlot, notes, instructions } = req.body;
    const lead = await MovingLead.create({
      ...
      notes: [serviceType ? `Service: ${serviceType}` : '', propertyType ? `Property: ${propertyType}` : '', pickupFloor ? `Pickup floor: ${pickupFloor}${pickupElevator ? ' (elevator)' : ''}` : '', deliveryFloor ? `Delivery floor: ${deliveryFloor}${deliveryElevator ? ' (elevator)' : ''}` : '', timeSlot ? `Time: ${timeSlot}` : '', instructions || '', notes || ''].filter(Boolean).join('\n'),
...
router.post('/moves/:id/photos', upload.array('images', 20), async (req, res) => {
  try {
    ...
    const images = (req.files || []).map((f) => ({ url: `/uploads/${f.filename}`, filename: f.filename, originalName: f.originalname, size: f.size, category }));
