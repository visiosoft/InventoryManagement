...
export function generateMovingInvoicePdf(invoice) {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'A4', margin: 0, info: { Title: invoice.invoiceNo || 'Moving Invoice' } });
    const chunks = [];
    ...
    if (invoice.status === 'paid') {
      doc.save();
      doc.rotate(-30, { origin: [PW / 2, 130] });
      doc.font('Helvetica-Bold').fontSize(60).fillColor(PAID_GREEN).opacity(0.15)
        .text('PAID', M, 80, { width: PW - 2 * M, align: 'center' });
      doc.restore();
    }
    ...
    if (invoice.customer?.phone) {
      doc.font('Helvetica').fontSize(9).fillColor(BLACK).text(invoice.customer.phone, LX, ly, { width: LW });
      ly = doc.y + 6;
    }
    if (invoice.customer?.email) {
      doc.font('Helvetica').fontSize(9).fillColor(BLACK).text(invoice.customer.email, LX, ly, { width: LW });
      ly = doc.y + 8;
    }
    if (invoice.customer?.address) {
      doc.font('Helvetica').fontSize(9).fillColor(BLACK).text(invoice.customer.address, LX, ly, { width: LW });
      ly = doc.y + 8;
    }

    doc.font('Helvetica').fontSize(9).fillColor(GRAY).text('Invoice Date :', LX, ly, { width: LW });
    doc.font('Helvetica').fontSize(9).fillColor(BLACK).text(dt(invoice.invoiceDate), LX + 95, ly, { width: LW - 95 });
    ly += 16;
    doc.font('Helvetica').fontSize(9).fillColor(GRAY).text('Due Date :', LX, ly, { width: LW });
    doc.font('Helvetica').fontSize(9).fillColor(BLACK).text(dt(invoice.dueDate), LX + 95, ly, { width: LW - 95 });
    ly += 16;

    if (invoice.job?.jobNo) {
      doc.font('Helvetica').fontSize(9).fillColor(GRAY).text('Job No :', LX, ly, { width: LW });
      doc.font('Helvetica').fontSize(9).fillColor(BLACK).text(invoice.job.jobNo, LX + 95, ly, { width: LW - 95 });
      ly += 16;
    }
    ...
    if (invoice.job?.pickupAddress || invoice.job?.deliveryAddress) {
      doc.moveTo(M, y).lineTo(PW - M, y).strokeColor(BORDER).lineWidth(0.5).stroke();
      y += 10;
      const halfW = (PW - 2 * M - 20) / 2;
      if (invoice.job?.pickupAddress) {
        doc.font('Helvetica-Bold').fontSize(8).fillColor(GRAY).text('Pickup', M, y);
        doc.font('Helvetica').fontSize(9).fillColor(BLACK).text(invoice.job.pickupAddress, M, y + 10, { width: halfW });
      }
      if (invoice.job?.deliveryAddress) {
        doc.font('Helvetica-Bold').fontSize(8).fillColor(GRAY).text('Delivery', M + halfW + 20, y);
        doc.font('Helvetica').fontSize(9).fillColor(BLACK).text(invoice.job.deliveryAddress, M + halfW + 20, y + 10, { width: halfW });
      }
      y = doc.y + 16;
    }
    ...
    if (invoice.depositPaid) {
      doc.font('Helvetica').fontSize(9).fillColor(GRAY).text('Deposit Paid', tX, y, { width: lblW, align: 'right' });
      doc.font('Helvetica').fontSize(9).fillColor(BLACK).text(`-${num(invoice.depositPaid)}`, valX, y, { width: valW, align: 'right' });
      y += 16;
    }

    const paidAmount = (invoice.paymentHistory || []).reduce((s, p) => s + p.amount, 0) + (invoice.depositPaid || 0);
    if (paidAmount > 0) {
      doc.font('Helvetica').fontSize(9).fillColor(GRAY).text('Payments Made', tX, y, { width: lblW, align: 'right' });
      doc.font('Helvetica').fontSize(9).fillColor(PAID_GREEN).text(`-${num(paidAmount - (invoice.depositPaid || 0))}`, valX, y, { width: valW, align: 'right' });
      y += 16;
    }
    ...
    if (invoice.bankInformation) {
      doc.moveTo(M, y).lineTo(PW - M, y).strokeColor(BORDER).lineWidth(0.5).stroke();
      y += 12;
      doc.font('Helvetica-Bold').fontSize(9).fillColor(GRAY).text('Bank Information', M, y);
      y += 13;
      doc.font('Helvetica').fontSize(8).fillColor(DARK).text(invoice.bankInformation, M, y, { width: PW - 2 * M });
      y = doc.y + 12;
    }
    ...
    if (invoice.notes) {
      ...
      y += 13;
      doc.font('Helvetica').fontSize(8).fillColor(DARK).text(invoice.notes, M, y, { width: PW - 2 * M });
      y = doc.y + 12;
    }
